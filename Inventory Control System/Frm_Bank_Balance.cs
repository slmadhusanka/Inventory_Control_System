    ...
    public partial class Frm_Bank_Balance : Form
    {
        ...
        public void Load_Bank_ACC_Numbers()
        {
            ...
            try
            {
                ...
                string Bank_Name = @"SELECT Account_Num FROM Bank_Account_Details WHERE BankID='" + Bank_ID.Text + "'";
                SqlCommand cmd1 = new SqlCommand(Bank_Name, con1);
                SqlDataReader dr1 = cmd1.ExecuteReader(CommandBehavior.CloseConnection);
        ...
        public void Concat_SQL_Quary()
        {
            try
            {
                ...
                ReSelectQ = @"  SELECT     Bank_Balance.Bal_Auto_ID, Bank_Balance.DoC_ID, Bank_Doc_details.Bank_Name, Bank_Doc_details.Acc_Num, Bank_Balance.Amount_Status, 
                      Bank_Balance.Debit_Amount, Bank_Balance.Credit_Amount, Bank_Balance.Balance, Bank_Balance.Add_User, Bank_Doc_details.Banked_Date, 
                      Bank_Doc_details.Doc_Ref_ID, Bank_Category.BankName
                      FROM         Bank_Balance INNER JOIN
                      Bank_Doc_details ON Bank_Balance.DoC_ID = Bank_Doc_details.DoC_ID INNER JOIN
                ...
                {
                    ReSelectQ += " AND Bank_Balance.Add_User='" + Cmb_User.Text + "'";
                }
                ...
                if (by_Document_ID.SelectedIndex == 1)
                {
                    ReSelectQ = @" SELECT     Bank_Balance.Bal_Auto_ID, Bank_Balance.DoC_ID, Bank_Doc_details.Bank_Name, Bank_Doc_details.Acc_Num, Bank_Balance.Amount_Status, 
                      ...
                      FROM         Bank_Balance INNER JOIN
                      Bank_Doc_details ON Bank_Balance.DoC_ID = Bank_Doc_details.DoC_ID INNER JOIN
                      Bank_Category ON Bank_Doc_details.Bank_Name = Bank_Category.BankID WHERE 1=1 AND Bank_Balance.DoC_ID ='" + Cmb_Document_ID.Text + "'";
                }
