    ...
    public partial class Frm_Petty_cash : Form
    {
        ...
        public void Select_Users()
        {
            try
            {
                ...
                string Pety_Reson = @"SELECT  UserCode, DisplayOn FROM  UserProfile WHERE AtiveDeactive='1'";
                SqlCommand cmd1 = new SqlCommand(Pety_Reson, con1);
                SqlDataReader dr1 = cmd1.ExecuteReader(CommandBehavior.CloseConnection);

                while (dr1.Read())
                {
                    Cmb_Payer.Items.Add(dr1[0].ToString());
                    Cmb_Reciever.Items.Add(dr1[0].ToString());
                }
        ...
        public void Concat_SQL_Quary()
        {
            try
            {
                ...
                if (by_Pettycash_ID.SelectedIndex != 1)
                {
                    ReSelectQ += "AND ptDate BETWEEN '" + PickerDateFrom.Text + "' AND '" + PickerDateTo.Text + "' ";
                }
